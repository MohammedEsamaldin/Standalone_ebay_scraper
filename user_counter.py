...
def load_counters(filename= os.path.join(os.path.dirname(__file__), 'user_counter.json')
        ):
    try:
...
def save_counters(data , filename =os.path.join(os.path.dirname(__file__), 'user_counter.json')
        ):

    with open(filename, 'w') as file:
        json.dump(data, file, indent=4)

def update_counter(user , filename =os.path.join(os.path.dirname(__file__), 'user_counter.json'), max_count=5000):
...
def get_current_count(user,  filename = os.path.join(os.path.dirname(__file__), 'user_counter.json')
        ):
    counters = load_counters(filename)
    user_str = str(user)
...
def change_user_count(user, filename = os.path.join(os.path.dirname(__file__), 'user_counter.json')
        ):
    counters = load_counters(filename)
    user_str = str(user)
